...
def get_highvar_genes_sparse(
    expression, expected_fano_threshold=None, minimal_mean=0.5, numgenes=None
):
...
class cNMF:
    ...
    def _initialize_dirs(self):
        if self.paths is None:
            ...
            self.paths = {
                ...
                "normalized_counts_npz": os.path.join(
                    self.output_dir,
                    self.name,
                    "cnmf_tmp",
                    self.name + ".norm_counts.df.npz",
                ),
                ...
                "starcat_spectra": os.path.join(
                    self.output_dir,
                    self.name,
                    "cnmf_tmp",
                    self.name + ".starcat_spectra.k_%d.dt_%s.df.npz",
                ),
                "starcat_spectra__txt": os.path.join(
                    self.output_dir,
                    self.name,
                    self.name + ".starcat_spectra.k_%d.dt_%s.txt",
                ),
    ...
    def prepare(
        ...
        genes_file : str or list or None, optional (default=None)
        ...
        """
        if isinstance(counts_fn, ad.AnnData):
            input_counts = counts_fn
            if layer_key is not None:
                logger.info("Using layer %s from AnnData object." % layer_key)
                input_counts.X = input_counts.layers[layer_key]
        # elif isinstance(counts_fn, sp.csr.csr_matrix):
        #     input_counts = sc.AnnData(
        #         X=counts_fn,
        #         obs=pd.DataFrame(index=counts_fn.index),
        #         var=pd.DataFrame(index=counts_fn.columns),
        #     )
        elif isinstance(counts_fn, str):
            ...
            raise TypeError(
                "counts_fn must be a string (file path) or an AnnData object."
            )
        ...
        if tpm_fn is None:
            ...
            _write_h5ad_safe(self.paths["tpm"], tpm)
        ...
        if sp.issparse(tpm.X):
            ...
            gene_tpm_stddev = gene_tpm_stddev**0.5
        else:
        ...
        if isinstance(genes_file, str):
            if not os.path.isfile(genes_file):
                raise FileNotFoundError(f"File not found: {genes_file}")
            with open(genes_file) as f:
                highvargenes = f.read().rstrip().split("\n")

        elif isinstance(genes_file, list):
            if not genes_file:
                raise ValueError("The gene list is empty.")
            if len(genes_file) != num_highvar_genes:
                raise ValueError(
                    f"Expected {num_highvar_genes} genes, got {len(genes_file)}."
                )
            highvargenes = genes_file
        elif genes_file is None:
            highvargenes = None
        else:
            raise TypeError(
                "genes_file must be a string (file path), list of genes, or None."
            )
    ...
    def get_norm_counts(
        ...
        if zerocells.sum() > 0:
            ...
            raise Exception(
                "Error: %d cells have zero counts of overdispersed genes. E.g. %s. Filter those cells and re-run or adjust the number of overdispersed genes. Quitting!"
    ...
    def save_norm_counts(self, norm_counts):
        self._initialize_dirs()
        _write_h5ad_safe(self.paths["normalized_counts"], norm_counts)
        # sc.write(self.paths["normalized_counts"], norm_counts)
        norm_counts_X = (
            norm_counts.X.toarray() if sp.issparse(norm_counts.X) else norm_counts.X
        )
        obs_names = np.array(norm_counts.obs_names)
        var_names = np.array(norm_counts.var_names)
        np.savez(
            self.paths["normalized_counts_npz"],
            X=norm_counts_X,
            obs_names=obs_names,
            var_names=var_names,
        )
    ...
    def get_nmf_iter_params(
        ...
        beta_loss : str, optional (default='kullback-leibler')
            Loss function to use for NMF. Can be 'frobenius', 'kullback-leibler', or 'itakura-saito'.
            See sklearn documentation for details.
        init : str, optional (default='random')
            Initialization method for NMF. Can be 'random', 'nndsvd', or 'nndsvda'.
        max_iter : int, optional (default=1000)
            Maximum number of iterations for each NMF run.
        verbose : int, optional (default=0)
            Verbosity level for NMF. 0 for no output, 1 for progress messages.
        """
        ...
        replicate_params = pd.DataFrame(
            replicate_params, columns=["n_components", "iter", "nmf_seed", "completed"]
        )

        n_completed = replicate_params["completed"].sum()
        if n_completed > 0:
            message = """{n} runs already appear completed. If this is unexpected, consider
            re-initializing the cnmf object with a different run name or output directory""".format(
                n=n_completed
            )
    ...
    def factorize(
        ...
        with np.load(self.paths["normalized_counts_npz"], allow_pickle=True) as f:
            X = f["X"]
            obs_names = f["obs_names"]
            var_names = f["var_names"]
            norm_counts = pd.DataFrame(X, index=obs_names, columns=var_names)

        # print(f"Type: --- {type(norm_counts)} ---")
        ...
        if not skip_completed_runs:
            ...
            jobs_for_this_worker = worker_filter(
                run_params.index[run_params["completed"] == False],
                worker_i,
                total_workers,
            )
        ...
        for idx in jobs_for_this_worker:
            ...
            (spectra, usages) = self._nmf(norm_counts.values, _nmf_kwargs)
    ...
    def combine_nmf(
        self, k, skip_missing_files=False, remove_individual_iterations=False
    ):
        run_params = load_df_from_npz(self.paths["nmf_replicate_parameters"])
        ...
        for i, p in run_params_subset.iterrows():
            ...
            if not os.path.exists(current_file):
                if not skip_missing_files:
                    print(
                        "Missing file: %s, run with skip_missing=True to override"
                        % current_file
                    )
                    raise FileNotFoundError(
                        errno.ENOENT, os.strerror(errno.ENOENT), current_file
                    )
                else:
                ...
                spectra.index = [
                    "iter%d_topic%d" % (p["iter"], t + 1) for t in range(k)
                ]
    ...
    def refit_usage(self, X, spectra):
        ...
        if type(spectra) is pd.DataFrame:
            refit_nmf_kwargs.update(
                dict(n_components=spectra.shape[0], H=spectra.values, update_H=False)
            )
        else:
            refit_nmf_kwargs.update(
                dict(n_components=spectra.shape[0], H=spectra, update_H=False)
            )
    ...
    def consensus(
        ...
        if not skip_density_and_return_after_stats:
            ...
            if l2_spectra.shape[0] == 0:
                raise RuntimeError(
                    "Zero components remain after density filtering. Consider increasing density threshold"
                )
        ...
        if skip_density_and_return_after_stats:
            ...
            if sp.issparse(norm_counts.X):
                ...
                prediction_error = (
                    ((norm_counts.X - rf_pred_norm_counts) ** 2).sum().sum()
                )
        ...
        spectra_tpm = pd.DataFrame(
            spectra_tpm, index=rf_usages.columns, columns=tpm.var.index
        )
        ...
        if refit_usage:
            ...
            rf_usages = self.refit_usage(
                norm_tpm.X, spectra_tpm_rf.astype(norm_tpm.X.dtype)
            )
            rf_usages = pd.DataFrame(
                rf_usages, index=norm_counts.obs.index, columns=spectra_tpm_rf.index
            )
    ...
    def build_reference(self, k, density_threshold=0.5, target_sum=1e6):
        ...
        spectra_tpm_renorm = (
            spectra_tpm_renorm.div(spectra_tpm_renorm.sum(axis=1), axis=0) * target_sum
        )
        ...
        save_df_to_npz(
            ref_spectra, self.paths["starcat_spectra"] % (k, density_threshold_repl)
        )
        save_df_to_text(
            ref_spectra,
            self.paths["starcat_spectra__txt"] % (k, density_threshold_repl),
        )
    ...
    def load_results(self, K, density_threshold, n_top_genes=100, norm_usage=True):
        ...
        """
        scorefn = self.paths["gene_spectra_score__txt"] % (
            K,
            str(density_threshold).replace(".", "_"),
        )
        tpmfn = self.paths["gene_spectra_tpm__txt"] % (
            K,
            str(density_threshold).replace(".", "_"),
        )
        usagefn = self.paths["consensus_usages__txt"] % (
            K,
            str(density_threshold).replace(".", "_"),
        )
        ...
        for gep in spectra_scores.columns:
            top_genes.append(
                list(
                    spectra_scores.sort_values(by=gep, ascending=False).index[
                        :n_top_genes
                    ]
                )
            )
...
def main():
    ...
    parser.add_argument(
        ...
        "--counts",
        type=str,
        help="[prepare] Input (cell x gene) counts matrix as .h5ad, .mtx, df.npz, or tab delimited text file",
    )
    parser.add_argument(
        "-k",
        "--components",
    ...
    parser.add_argument(
        "--max-nmf-iter",
        type=int,
        help="[prepare] Max number of iterations per individual NMF run (default 1000)",
        default=1000,
    )
    parser.add_argument(
        "--beta-loss",
        ...
        help="[prepare] Loss function for NMF (default frobenius)",
        default="frobenius",
    )
    parser.add_argument(
        "--init",
        ...
        help="[prepare] Initialization algorithm for NMF (default random)",
        default="random",
    )
    parser.add_argument(
        "--densify",
        dest="densify",
        help="[prepare] Treat the input data as non-sparse (default False)",
        action="store_true",
        default=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--skip-completed-runs",
        action="store_true",
        help="[factorize] Skip previously completed runs. Must re-run prepare first to update completed runs",
        default=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--build-reference",
        dest="build_reference",
        help="[consensus] Generates a reference spectra for use in starCAT",
        action="store_true",
        default=True,
    )
